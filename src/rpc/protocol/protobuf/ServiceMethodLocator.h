#ifndef _SRC_RPC_PROTOCOL_PROTOBUF_SERVICE_METHOD_LOCATOR_H_
#define _SRC_RPC_PROTOCOL_PROTOBUF_SERVICE_METHOD_LOCATOR_H_
...
class ServiceMethodLocator : public tinyRPC::Singleton<ServiceMethodLocator>{
  template <class T>
 ...
 public:
  ...
  using LocatorProviderCallback =
      UniqueFunction<void(const google::protobuf::MethodDescriptor* method)>;
 ...
 private:
  template <class T>
  struct ControlBlock {
    std::unordered_map<MethodKey<T>, MethodDesc<T>> key_desc_map;
  ...
  ControlBlock<T>* GetCachedControlBlock() const {
    thread_local std::uint64_t version = 0;
