#ifndef _SRC_BASE_STRING_H_
#define _SRC_BASE_STRING_H_
...
//       std::common_type<T>, std::string_view s, ...);
//
// ... or specialize `TryParseTraits<T>`.
//
// FIXME: The first approach does not work yet.
template <class T, class... Args>
inline std::optional<T> TryParse(std::string_view s, const Args&... args) {
...
std::vector<std::string_view> Split(std::string_view s, std::string_view delim,
                                    bool keep_empty = false);
// void Split(std::string_view s,
//            Function<std::size_t(std::string_view)> finder,
...
struct TryParseTraits<T, std::enable_if_t<std::is_integral_v<T>>> {
  static std::optional<T> TryParse(std::string_view s, int base = 10);
};
