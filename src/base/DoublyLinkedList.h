#ifndef _SRC_BASE_DOUBLY_LINKED_LIST_H_
#define _SRC_BASE_DOUBLY_LINKED_LIST_H_ 
...
constexpr T& DoublyLinkedList<T, kEntry>::front() noexcept {
  CHECK_NE(head_.next, &head_) << "Calling `front()` on empty list is undefined.\n";
...
constexpr T& DoublyLinkedList<T, kEntry>::back() noexcept {
  CHECK_NE(head_.prev, &head_) << "Calling `back()` on empty list is undefined.\n";
...
constexpr T* DoublyLinkedList<T, kEntry>::pop_front() noexcept {
  if (head_.prev == &head_) {
    // Empty then.
    CHECK_EQ(head_.prev, head_.next);
    return nullptr;
  }
  auto rc = head_.next;
...
#endif  // _SRC_BASE_DOUBLY_LINKED_LIST_H_
