#ifndef _SRC_IO_EVENT_LOOP_H_
#define _SRC_IO_EVENT_LOOP_H_
...
#include "detail/EventLoopNotifier.h"
...
class EventLoop {
 public:
  ...
  void AttachDescriptor(DescriptorPtr desc, bool enabled = true);
  ...
  void DetachDescriptor(DescriptorPtr desc);
  ...
  void AddTask(UniqueFunction<void()>&& cb);
 ...
 private:
  ...
  // problem.
  std::list<UniqueFunction<void()>> tasks_;

  std::mutex desc_lock_;
  std::set<DescriptorPtr> descriptors_;
};
